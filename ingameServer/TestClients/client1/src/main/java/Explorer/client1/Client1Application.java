...
public class Client1Application {
	private static final Logger log = LoggerFactory.getLogger(Client1Application.class);    // Logger
	...
	public static void main(String[] args) {
		...
		try (Socket socket = new Socket(IpAddress, PORT)) {
			 ...
			 */
			System.out.println("Enter your JSON message:");
			// (1) Thread 객체 생성
			Thread inputThread = new Thread(() -> {
				try {
					...
					while((userInput = stdIn.readLine()) != null) {     // 1) 사용자가 콘솔에 데이터를 입력
						...
						System.out.println("Enter your message:"); // 3) 다음 입력을 기다리는 메시지 출력
					}
